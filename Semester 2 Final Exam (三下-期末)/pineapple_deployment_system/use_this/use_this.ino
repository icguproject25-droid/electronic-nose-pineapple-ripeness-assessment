...
const int TGS2620_PIN = A5;

const unsigned long SAMPLE_INTERVAL_MS = 1000;
const unsigned long PREHEAT_TIME_MS = 0;
const unsigned long DELAY_START_SEC = 0;
...
SensorData readAllSensors() {
  ...
  d.tgs2620_raw = analogRead(TGS2620_PIN);
...
// -------- 最終統計用 --------
long sum_mq2 = 0, sum_mq3 = 0, sum_mq9 = 0, sum_mq135 = 0, sum_tgs = 0, sum_tgs2620 = 0;
float sum_temp = 0, sum_hum = 0, sum_press = 0;

int min_mq2 = 9999, min_mq3 = 9999, min_mq9 = 9999, min_mq135 = 9999, min_tgs = 9999, min_tgs2620 = 9999;
int max_mq2 = 0,    max_mq3 = 0,    max_mq9 = 0,    max_mq135 = 0,    max_tgs = 0,    max_tgs2620 = 0;
...
long base_sum_mq2 = 0, base_sum_mq3 = 0, base_sum_mq9 = 0, base_sum_mq135 = 0, base_sum_tgs = 0, base_sum_tgs2620 = 0;
float base_sum_temp = 0, base_sum_hum = 0, base_sum_press = 0;

float base_mq2 = 0, base_mq3 = 0, base_mq9 = 0, base_mq135 = 0, base_tgs = 0, base_tgs2620 = 0;
float base_temp = 0, base_hum = 0, base_press = 0;

// -------- delta --------
bool hasPrev = false;
float prev_l_mq2 = 0, prev_l_mq3 = 0, prev_l_mq9 = 0, prev_l_mq135 = 0, prev_l_tgs = 0, prev_l_tgs2620 = 0;

// -------- rolling MA5 --------
float buf_l_mq2[MA_WIN], buf_l_mq3[MA_WIN], buf_l_mq9[MA_WIN], buf_l_mq135[MA_WIN], buf_l_tgs[MA_WIN], buf_l_tgs2620[MA_WIN];
int bufIdx = 0;
int bufFilled = 0;
...
void loop() {
  ...
  if (!preheatDone) {
    ...
    if (elapsed >= PREHEAT_TIME_MS) {
      ...
      Serial.println(
        "timestamp_ms,"
        "timestamp_s,"
        "MQ2_raw,MQ3_raw,MQ9_raw,MQ135_raw,TGS2602_raw,TGS2620_raw,"
        "Temp_C,Humidity_pct,Pressure_hPa,"
        "MQ2_logr,MQ3_logr,MQ9_logr,MQ135_logr,TGS2602_logr,TGS2620_logr,"
        "MQ2_dlogr,MQ3_dlogr,MQ9_dlogr,MQ135_dlogr,TGS2602_dlogr,TGS2620_dlogr,"
        "MQ2_logr_ma5,MQ3_logr_ma5,MQ9_logr_ma5,MQ135_logr_ma5,TGS2602_logr_ma5,TGS2620_logr_ma5"
      );
  ...
  if (!isValidMQRaw(d.mq2_raw) || !isValidMQRaw(d.mq3_raw) || 
    ...
    Serial.print(d.tgs2620_raw); Serial.print(" T=");
  ...
  if (!baselineReady) {
    ...
    base_sum_tgs2620 += d.tgs2620_raw;
    ...
    if (baselineCount >= BASELINE_SAMPLES) {
      ...
      base_tgs2620 = base_sum_tgs2620 / (float)baselineCount;
      ...
      Serial.print(" TGS2620="); Serial.print(base_tgs2620, 2);
  ...
  min_tgs2620 = min(min_tgs2620, d.tgs2620_raw);    max_tgs2620 = max(max_tgs2620, d.tgs2620_raw);
  ...
  float btgs2620 = baselineReady ? base_tgs2620 : (float)d.tgs2620_raw;
  ...
  float l_tgs2620 = logf(((float)d.tgs2620_raw + 1.0f) / (btgs2620 + 1.0f));

  float dl_mq2 = 0, dl_mq3 = 0, dl_mq9 = 0, dl_mq135 = 0, dl_tgs = 0, dl_tgs2620 = 0;
  if (hasPrev) {
    ...
    dl_tgs2620 = l_tgs2620 - prev_l_tgs2620;
  } else {
    hasPrev = true;
  }
  prev_l_mq2 = l_mq2; prev_l_mq3 = l_mq3; prev_l_mq9 = l_mq9; prev_l_mq135 = l_mq135; prev_l_tgs = l_tgs; prev_l_tgs2620 = l_tgs2620;
  ...
  buf_l_tgs2620[bufIdx] = l_tgs2620;
  ...
  float ma_tgs2620 = meanBuf(buf_l_tgs2620, bufFilled);
  ...
  Serial.print(d.mq135_raw); Serial.print(",");
  Serial.print(d.tgs2602_raw); Serial.print(",");
  Serial.print(d.tgs2620_raw); Serial.print(",");
  ...
  Serial.print(l_tgs2620, 6); Serial.print(",");
  ...
  Serial.print(dl_tgs2620, 6); Serial.print(",");
  ...
  Serial.print(ma_tgs, 6); Serial.print(",");
  Serial.println(ma_tgs2620, 6);
  ...
  if (sampleCount >= MAX_SAMPLES) {
    ...
    float avg_tgs2620 = sum_tgs2620 / (float)sampleCount;
    ...
    Serial.print("# TGS2620 avg="); Serial.print(avg_tgs2620, 2); Serial.print(" min="); Serial.print(min_tgs2620); Serial.print(" max="); Serial.println(max_tgs2620);
