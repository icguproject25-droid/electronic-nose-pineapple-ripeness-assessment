...
GitHub README.md 可以直接引用同資料夾內的圖片。  
若圖片與 README.md 放在同一個 `08-class-diagram/` 資料夾中，可以使用以下格式：

```md
![圖片說明](./圖片檔名.png)
```

因為本資料夾的圖片檔名包含空白、中文與 `&` 符號，建議在圖片路徑中：
...
```md
![8-3 使用者掃描結果](./8-3%20使用者%26掃描紀錄%26成熟度結果類別圖.png)
```

如果圖片無法顯示，請檢查：

1. 圖片是否與 README.md 在同一個資料夾。
2. 檔名是否完全相同。
3. 空白是否改成 `%20`。
4. `&` 是否改成 `%26`。
5. 副檔名是否為 `.png`，大小寫是否一致。
6. GitHub 是否已成功上傳圖片。
...
若你同時保留 PlantUML 原始碼，也可以改成：

```text
08-class-diagram/
├── README.md
├── puml/
│   ├── 8-1-overall-class-diagram-CH.puml
│   ├── 8-1-overall-class-diagram-EN.puml
│   ├── 8-2-overall-summary-class-diagram.puml
│   ├── 8-3-user-scan-result-class-diagram.puml
│   ├── 8-4-farmer-batch-report-class-diagram.puml
│   ├── 8-5-system-model-class-diagram.puml
│   └── 8-6-photo-recognition-class-diagram.puml
└── images/
    ├── 8-1 Ch08 整體資訊系統類別圖_CH.png
    ├── 8-1 Ch08 整體資訊系統類別圖_EN.png
    ├── 8-2 整體資訊系統類別總覽圖.png
...
若採用 `images/` 子資料夾，README 圖片路徑要改成：

```md
![8-1 中文版](./images/8-1%20Ch08%20整體資訊系統類別圖_CH.png)
```
